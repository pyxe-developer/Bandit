...
**Current next action:** Run Stage 4 pre-landing review for `BANDIT-060`:
...
until Stage 4 evidence is recorded and current.

The current stage is Stage 4 review pending. Stage 3 implementation is accepted
for review, but `BANDIT-060` is not safe-to-land and is not landed.
...
passes after Test Writer contract reconciliation. The next stage is Stage 4
pre-landing review.
...
1. Run Stage 4 pre-landing review for `BANDIT-060`: CodeRabbit pre-PR review,
...
No operator-owned input is required for the recorded Stage 4 review next action.
