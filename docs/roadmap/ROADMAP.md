...
**Current next step:** Create RED evidence for `BANDIT-015` - Live CodeRabbit
Pre-Landing Loop. `BANDIT-014` resolved the Landing Agent bootstrap gap by
adding a repo-native contract, validation path, and local-record landing
command. `BANDIT-015` is now active for `BANDIT-GAP-LIVE-CODERABBIT`. Open
...
- Create RED evidence for `BANDIT-015` - Live CodeRabbit Pre-Landing Loop.
...
- `BANDIT-GAP-LIVE-CODERABBIT` is active as `BANDIT-015`.
