...
**Current next step:** Implement the narrow heartbeat chore-agent policy parser
and `bandit heartbeat inspect` command for active chore `BANDIT-022`
...
`docs/work/BANDIT-022/red-evidence.md`, and `.bandit/bootstrap-gaps.json` links
the gap to `BANDIT-022` as `active_chore`.
...
active at Stage 2 with implementation next.
