...
coderabbit_replacement_evidence:
  - Real pre-PR CodeRabbit provider evidence was retried against the repaired source and completed with 2 unresolved findings; no replacement pass is claimed.
...
pm_disposition_rationale: Stage 4 remains blocked because the repaired-source CodeRabbit pre-PR retry completed with 2 unresolved findings. The previous eight findings have repair/disposition evidence, but the new design-canvas scale guard and Babel standalone version/integrity findings must be repaired or dispositioned before Local Qwen, review-subject hash refresh if needed, and aggregate PM disposition can proceed.
