...
- The brief identifies `BANDIT-PRD-004`, the PRD-004/005 decomposition, and
  closed `BANDIT-089` schema-contract evidence as source authority.
- The scope is bounded to `BANDIT-PRD-004.2` Attribution Join Key contracts,
  validation helpers, hash derivation, templates, and landing-validation
  integration. It does not start escape workflow, boundary movement,
  `BANDIT-PRD-005`, the V0 Closeout Claude Code A/B Product-Value Trial, Trust
  Verifier cutover, model gateway, telemetry, local API, State Index, cockpit
  UI, hosted services, paid routing, merge, push, or deploy work.
- `CLEAN_CODE.md` read evidence is recorded and the acceptance criteria are
  shaped around small, explicit, testable tuple parsing, hash derivation,
  landing integration, templates, and failure paths.
...
- Operator-owned gates are explicit. No operator-owned input is required for
  ...
  product/UAT direction changes, paid/live routing, hosted services, telemetry,
  merge/push/deploy authority, Trust Verifier cutover, public benchmark claims,
  business tradeoffs, policy changes, or explicit cost/risk decisions.
...
plan-mode orchestration for `BANDIT-090`; Repo PM must stop after
