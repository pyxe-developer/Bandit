# CodeRabbit Formation Review - BANDIT-072
...
- replay-only boundary preserved: not_applicable - CodeRabbit did not return a terminal review verdict.
...
Results: CLI version `0.5.3`; agent auth returned authenticated for the current GitHub account.
...
CodeRabbit formation review for `BANDIT-072` is unavailable due to bounded
