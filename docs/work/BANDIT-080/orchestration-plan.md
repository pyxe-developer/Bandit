# BANDIT-080 Orchestration Plan

contract_version: 1
work_item: BANDIT-080
work_type: slice
orchestrator: work_item_pm
created_at: 2026-06-09T01:34:38Z

## Current Repo State

`BANDIT-080` is the active Phase 8 product slice for Queue & Context (Light).
Stage 1 formation is complete: `docs/work/BANDIT-080/brief.md`,
`docs/work/BANDIT-080/qwen-formation-review.md`,
`docs/work/BANDIT-080/coderabbit-formation-review.md`,
`docs/work/BANDIT-080/formation-review.md`, and
`docs/work/BANDIT-080/coordination-log.jsonl` exist, and the coordination log
...
and `node ./bin/bandit.mjs work-item-pm start BANDIT-080` records
...
| --- | --- | --- |
| Stage 2 RED | Test Writer | RED tests and RED evidence map queue/context derivation, current/next/planned/deferred states, recent coordination context, fail-closed missing states, source traceability, browser authority boundaries, responsive behavior, and accessibility states to acceptance criteria. |
...
| Stage | Evidence |
| --- | --- |
| Stage 2 | `test/cockpit-queue-context.test.mjs`, relevant cockpit/view-model/evidence/UI tests or fixtures, and `docs/work/BANDIT-080/red-evidence.md`. |
| Stage 3 | `docs/work/BANDIT-080/implementation-evidence.md`, `docs/work/BANDIT-080/writer-report.md`, `docs/work/BANDIT-080/stage3-pm-review.md`, and role/run or dispatch evidence. |
| Stage 4 | `docs/work/BANDIT-080/coderabbit-review.md`, `docs/work/BANDIT-080/local-qwen-review.md`, risk classification, supply-chain gate, review-subject hash, browser smoke evidence, finding dispositions when needed, and `docs/work/BANDIT-080/review-evidence.md`. |
| Stage 5 | `docs/work/BANDIT-080/uat-approval.md`, `docs/work/BANDIT-080/landing-verdict.md`, `docs/work/BANDIT-080/landing-action.md`, land-check output, and current source/evidence commit. |
| Stage 6 | `docs/work/BANDIT-080/retrospective.md`, improvement or no-action disposition artifacts, `.bandit/bootstrap-gaps.json` if changed, `docs/roadmap/CURRENT_CONTEXT.md`, `docs/roadmap/ROADMAP.md`, `STATUS.md`, and final validation output. |
...
- Permanent Test Ownership Boundary: Stage 3 Writer has zero authority to
  ...
  helpers, fixtures, RED evidence, or acceptance mappings for `BANDIT-080`.
...
- Operator owns product direction and any product, policy, business, cost,
  ...
  publication, or Trust Verifier cutover decision that repo artifacts cannot
  answer. Product UAT evidence is required before landing.
...
- `node ./bin/bandit.mjs coordination validate BANDIT-080`
- `npm run bandit -- coderabbit-review pre-pr BANDIT-080 --base origin/main`
- `npm run bandit -- qwen-review BANDIT-080`
- `npm run bandit -- review-subject-hash BANDIT-080`
- `npm run bandit -- land-check BANDIT-080`
- `git diff --check`
...
mobile text fit, source-path wrapping, queue row density, current/next labels,
planned/deferred chips, recent-transition rows, focus order, source-link
reachability, and no overlap or truncation.
...
| Product UAT before landing | non_blocking | `docs/work/BANDIT-080/brief.md` requires CLI-owned product UAT before landing; not required before Stage 2. |
...
- Browser code invokes CLI commands, writes repo artifacts, records UAT,
  decides landing safety, merges, pushes, deploys, changes policy, starts live
  guarded execution, schedules or claims work, or makes browser state canonical.
- Queue/context derivation silently normalizes missing, stale, contradictory,
  blocked, unavailable, deferred, or not-yet-formed states into complete or
  healthy backlog rows.
- UI text, chart labels, or row labels imply backlog management, priority
  control, scheduling authority, claimability, statistical proof, public
  benchmark approval, or automatic policy approval.
...
- Operator-owned product, UAT, business, policy, cost, risk, Trust Verifier
  cutover, guarded action execution, benchmark publication, merge, push,
  deploy, external-service, or genuinely ambiguous scope decision is required.

## Forbidden Actions

- Do not implement backlog editing, work intake triage, queue prioritization
  controls, kanban lanes, drag-and-drop planning, local queue management,
  scheduler execution, claim leases, work-surface reservations, worktree
  lifecycle, local API endpoints, State Index persistence, SQLite, live
  polling, websocket updates, browser-side CLI execution, guarded action
  execution, PR/CI orchestration, automatic merge, push, deploy, production
  canary behavior, hosted replay services, cross-repo aggregation, or external
  telemetry.
- Do not create the Operator Inbox/Operator Attention implementation or run the
  Claude Code A/B product-value trial in this slice.
- Do not create trial repos, choose the trial PRD, score trial results, publish
  benchmark claims, or use this slice to approve public benchmark publication.
- Do not choose local API shape, State Index timing, hosted packaging, external
  ...
  package-manager script changes, paid reviewer/model routing, or live action
  queue semantics.
- Do not let generated JSON, preview files, fixture data, browser state, local
  cache, screenshots, static HTML, CSS, UI component state, queue-context rows,
  or the view model become canonical workflow authority.
- Do not make the UI approve product UAT, policy overrides, business
  ...
  queue priority, work assignment, or landing safety.
...
| --- | --- | --- |
| Required reads complete | pass | Read `AGENTS.md`, `CONTEXT.md`, `docs/roadmap/CURRENT_CONTEXT.md`, `docs/roadmap/ROADMAP.md`, `docs/plans/BOOTSTRAP_METHODOLOGY.md`, `CLEAN_CODE.md`, `docs/verification/STAGE_RUBRICS.md`, `STATUS.md`, `docs/work/BANDIT-080/brief.md`, `docs/work/BANDIT-080/coordination-log.jsonl`, and `docs/templates/work-item-pm-plan.md`. |
| Git status inspected | pass | `git status --short --branch` reported `## main...origin/main` with no dirty paths before plan edits. |
| Routing surfaces agree | pass | `CURRENT_CONTEXT.md`, `ROADMAP.md`, `STATUS.md`, `cockpit status --json`, and `session-context current --json` identify `BANDIT-080` and plan mode as the next action. |
| Prior Work Item closed | pass | `docs/work/BANDIT-079/landing-action.md`, `docs/work/BANDIT-079/retrospective.md`, `docs/work/BANDIT-079/improvement-disposition.md`, and `docs/work/BANDIT-079/coordination-log.jsonl` show landed and closed evidence. |
...
| Check | Verdict | Evidence |
| --- | --- | --- |
| Brief exists and satisfies Stage 1 | pass | `docs/work/BANDIT-080/brief.md` includes product work, source provenance, scope, out-of-scope, acceptance criteria, test plan, CLEAN_CODE.md evidence, role boundaries, stage capability scope, expected files, and operator-input status. |
| Qwen formation review exists and is not blocking | pass | `docs/work/BANDIT-080/qwen-formation-review.md` records `verdict: pass` and `findings_status: none` through the authorized MLX adapter route. |
| CodeRabbit formation review exists and is not blocking | pass | `docs/work/BANDIT-080/coderabbit-formation-review.md` records `verdict: non_blocking`; three trivial existing-source readability findings are out of the Stage 1 formation subject and dispositioned as non-blocking for formation. |
| Aggregate formation review exists and is not blocking | pass | `docs/work/BANDIT-080/formation-review.md` records `verdict: pass` and no open Stage 1 blockers. Its approval-next-action wording predates sequence 2 in the coordination log and is superseded by the append-only `formation_approved` transition. |
| Coordination log records `formation_approved` | pass | `docs/work/BANDIT-080/coordination-log.jsonl` sequence 2 records `state":"formation_approved"`. |
...
| RED evidence maps to acceptance criteria | pass | This plan requires RED tests for queue/context derivation, current/next/planned/deferred state, V0 closeout planning item display, recent coordination context, fail-closed missing-state rendering, source traceability, browser authority boundaries, responsive behavior, and accessibility states before Stage 3. |
...
| Focused tests pass or bootstrap gap recorded | pass | Verification Commands list focused cockpit queue/context tests, related cockpit tests, `npm test` when shared surfaces are touched, and `npm run typecheck`. |
...
| Check | Verdict | Evidence |
| --- | --- | --- |
| CodeRabbit review or provider-refusal evidence | pass | Verification Commands include `npm run bandit -- coderabbit-review pre-pr BANDIT-080 --base origin/main`; brief allows honest timeout/refusal evidence. |
| Local Qwen review | pass | Verification Commands include `npm run bandit -- qwen-review BANDIT-080`; Local Qwen route is limited to `.bandit/reviewers/local-qwen.json` through `bin/omlx-chat-completions.mjs`. |
...
| Supply-chain gate when applicable | pass | Required Evidence includes supply-chain gate because browser-facing and planning-context presentation surfaces may be touched. |
| Findings repaired or dispositioned | pass | Stage 4 stop conditions block unresolved findings or stale evidence. |
| Aggregate review current for subject | pass | Verification Commands include `npm run bandit -- review-subject-hash BANDIT-080` and aggregate `review-evidence.md`. |
...
| Check | Verdict | Evidence |
| --- | --- | --- |
| Landing verdict exists before landing action | pass | Required Evidence lists `docs/work/BANDIT-080/landing-verdict.md`. |
| Feature UAT handled | pass | Brief and this plan require CLI-owned product UAT evidence before landing. |
| `land-check` passes | pass | Verification Commands include `npm run bandit -- land-check BANDIT-080`. |
| Landing action records commit SHA | pass | Required Evidence lists `docs/work/BANDIT-080/landing-action.md`; landing path is local-record after clean source/evidence commit and source-head/hash refresh. |
...
| Check | Verdict | Evidence |
| --- | --- | --- |
| Retrospective exists | pass | Required Evidence lists `docs/work/BANDIT-080/retrospective.md`. |
