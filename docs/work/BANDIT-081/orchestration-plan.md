# BANDIT-081 Orchestration Plan

contract_version: 1
work_item: BANDIT-081
work_type: slice
orchestrator: work_item_pm
created_at: 2026-06-09T04:02:00Z

## Current Repo State

`BANDIT-081` is the active Phase 8 product slice for the Operator Attention /
Operator Inbox Surface. Stage 1 formation is complete:
...
`docs/work/BANDIT-081/formation-review.md`, and
`docs/work/BANDIT-081/coordination-log.jsonl` exist, and the coordination log
...
and `node ./bin/bandit.mjs work-item-pm start BANDIT-081` records
...
| --- | --- | --- |
| Stage 2 RED | Test Writer | RED tests and RED evidence map operator-attention derivation, required-input/blocker/stale/source states, inbox empty/unavailable state, fixture-backed inbox messages, source traceability, no browser authority, responsive behavior, and accessibility states to acceptance criteria. |
...
| Stage | Evidence |
| --- | --- |
| Stage 2 | `test/cockpit-operator-attention.test.mjs`, relevant cockpit/view-model/evidence/UI tests or fixtures, and `docs/work/BANDIT-081/red-evidence.md`. |
| Stage 3 | `docs/work/BANDIT-081/stage3-dispatch.md`, `docs/work/BANDIT-081/implementation-evidence.md`, `docs/work/BANDIT-081/writer-report.md`, `docs/work/BANDIT-081/stage3-pm-review.md`, and role/run evidence. |
| Stage 4 | `docs/work/BANDIT-081/coderabbit-review.md`, `docs/work/BANDIT-081/local-qwen-review.md`, risk classification, supply-chain gate, review-subject hash, browser smoke evidence, finding dispositions when needed, and `docs/work/BANDIT-081/review-evidence.md`. |
| Stage 5 | `docs/work/BANDIT-081/uat-approval.md`, `docs/work/BANDIT-081/landing-verdict.md`, `docs/work/BANDIT-081/landing-action.md`, land-check output, and current source/evidence commit. |
| Stage 6 | `docs/work/BANDIT-081/retrospective.md`, improvement or no-action disposition artifacts, `.bandit/bootstrap-gaps.json` if changed, `docs/roadmap/CURRENT_CONTEXT.md`, `docs/roadmap/ROADMAP.md`, `STATUS.md`, and final validation output. |
...
- Permanent Test Ownership Boundary: Stage 3 Writer has zero authority to
  ...
  helpers, fixtures, RED evidence, or acceptance mappings for `BANDIT-081`.
...
- `node ./bin/bandit.mjs coordination validate BANDIT-081`
- `npm run bandit -- coderabbit-review pre-pr BANDIT-081 --base origin/main`
- `npm run bandit -- qwen-review BANDIT-081`
- `npm run bandit -- review-subject-hash BANDIT-081`
- `npm run bandit -- land-check BANDIT-081`
- `git diff --check`
...
mobile text fit, source-path wrapping, operator-attention cards, inbox message
rows, required-input labels, blocked reasons, empty/unavailable states, focus
order, source-link reachability, and no overlap or truncation.
...
| Product UAT before landing | non_blocking | `docs/work/BANDIT-081/brief.md` requires CLI-owned product UAT before landing; not required before Stage 2. |
...
- Browser code invokes CLI commands, writes repo artifacts, records UAT,
  ...
  guarded execution, schedules or claims work, resolves inbox messages, or
  makes browser state canonical.
- Operator-attention or Operator Inbox derivation silently normalizes required,
  blocked, stale, contradictory, unavailable, empty-inbox, resolved, or
  source-missing states into complete or healthy rows.
- UI text, chart labels, or row labels imply approval storage, notification
  authority, inbox write/resolve/archive authority, operator-response capture,
  scheduling authority, claimability, statistical proof, public benchmark
  approval, or automatic policy approval.
...
- Do not create, edit, resolve, archive, or delete Operator Inbox artifacts from
  the browser, static preview, view model, or tests except through future
  explicit CLI-owned artifact contracts.
- Do not implement notification delivery, proactive alerts, background polling,
  local API endpoints, websocket updates, State Index persistence, SQLite,
  browser storage as workflow state, server-side inbox/action behavior,
  browser-side CLI execution, guarded action execution, scheduler execution,
  claim leases, work-surface reservations, worktree lifecycle, PR/CI
  orchestration, automatic merge, push, deploy, production canary behavior,
  hosted replay services, cross-repo aggregation, or external telemetry.
- Do not create the V0 Closeout Claude Code A/B Product-Value Trial in this
  slice.
...
- Do not let generated JSON, preview files, fixture data, browser state, local
  cache, screenshots, static HTML, CSS, UI component state, operator-attention
  rows, inbox rows, or the view model become canonical workflow or Operator
  Inbox authority.
- Do not make the UI approve product UAT, policy overrides, business
  ...
  queue priority, work assignment, inbox resolution, or landing safety.
...
| Routing surfaces agree | pass | `CURRENT_CONTEXT.md`, `ROADMAP.md`, `STATUS.md`, `cockpit status --json`, and `session-context current --json` identify `BANDIT-081` and plan mode as the next action. |
| Prior Work Item closed | pass | `docs/work/BANDIT-080/landing-action.md`, `docs/work/BANDIT-080/retrospective.md`, `docs/work/BANDIT-080/improvement-disposition.md`, and recent git history show `BANDIT-080` landed and closed. |
...
| Check | Verdict | Evidence |
| --- | --- | --- |
| Brief exists and satisfies Stage 1 | pass | `docs/work/BANDIT-081/brief.md` includes product work, source provenance, scope, out-of-scope, acceptance criteria, test plan, CLEAN_CODE.md evidence, role boundaries, stage capability scope, expected files, and operator-input status. |
| Qwen formation review exists and is not blocking | pass | `docs/work/BANDIT-081/qwen-formation-review.md` records `verdict: pass` and `findings_status: none` through the authorized MLX adapter route. |
| CodeRabbit formation review exists and is not blocking | pass | `docs/work/BANDIT-081/coderabbit-formation-review.md` records `verdict: pass` and `findings_status: none`. |
| Aggregate formation review exists and is not blocking | pass | `docs/work/BANDIT-081/formation-review.md` records `verdict: pass` and no open Stage 1 blockers. |
| Coordination log records `formation_approved` | pass | `docs/work/BANDIT-081/coordination-log.jsonl` sequence 2 records `state":"formation_approved"`. |
...
| RED evidence maps to acceptance criteria | pass | This plan requires RED tests for operator-attention derivation, required input, blockers, stale/source states, inbox empty/unavailable states, fixture-backed inbox messages, source traceability, browser authority boundaries, responsive behavior, and accessibility before Stage 3. |
...
| Focused tests pass or bootstrap gap recorded | pass | Verification Commands list focused cockpit operator-attention tests, related cockpit tests, `npm test` when shared surfaces are touched, and `npm run typecheck`. |
...
| Check | Verdict | Evidence |
| --- | --- | --- |
| CodeRabbit review or provider-refusal evidence | pass | Verification Commands include `npm run bandit -- coderabbit-review pre-pr BANDIT-081 --base origin/main`; brief allows honest timeout/refusal evidence. |
| Local Qwen review | pass | Verification Commands include `npm run bandit -- qwen-review BANDIT-081`; Local Qwen route is limited to `.bandit/reviewers/local-qwen.json` through `bin/omlx-chat-completions.mjs`. |
...
| Supply-chain gate when applicable | pass | Required Evidence includes supply-chain gate because browser-facing and operator-decision presentation surfaces may be touched. |
| Findings repaired or dispositioned | pass | Stage 4 stop conditions block unresolved findings or stale evidence. |
| Aggregate review current for subject | pass | Verification Commands include `npm run bandit -- review-subject-hash BANDIT-081` and aggregate `review-evidence.md`. |
...
| Check | Verdict | Evidence |
| --- | --- | --- |
| Landing verdict exists before landing action | pass | Required Evidence lists `docs/work/BANDIT-081/landing-verdict.md`. |
| Feature UAT handled | pass | Brief and this plan require CLI-owned product UAT evidence before landing. |
| `land-check` passes | pass | Verification Commands include `npm run bandit -- land-check BANDIT-081`. |
| Landing action records commit SHA | pass | Required Evidence lists `docs/work/BANDIT-081/landing-action.md`; landing path is local-record after clean source/evidence commit and source-head/hash refresh. |
...
| Check | Verdict | Evidence |
| --- | --- | --- |
| Retrospective exists | pass | Required Evidence lists `docs/work/BANDIT-081/retrospective.md`. |
