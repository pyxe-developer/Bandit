...
`BANDIT-078` is the active Phase 8 product slice for Guarded CLI Action
Requests. Stage 1 formation is complete: `docs/work/BANDIT-078/brief.md`,
`docs/work/BANDIT-078/qwen-formation-review.md`,
`docs/work/BANDIT-078/coderabbit-formation-review.md`,
`docs/work/BANDIT-078/formation-review.md`, and
`docs/work/BANDIT-078/coordination-log.jsonl` exist, and the coordination log
...
and `node ./bin/bandit.mjs work-item-pm start BANDIT-078` records
...
| --- | --- | --- |
| Stage 2 RED | Test Writer | RED tests and RED evidence map guarded action requests to acceptance criteria. |
...
| Stage 4 Review | Reviewer / Codex PM aggregate | CodeRabbit, Local Qwen, risk, supply-chain, browser smoke, hash, and finding disposition evidence. |
| Stage 5 Landing | Landing Agent | UAT, landing verdict, land-check, local-record landing action. |
...
| Stage | Evidence |
| --- | --- |
| Stage 2 | `test/cockpit-actions.test.mjs`, relevant cockpit/UI tests or fixtures, and `docs/work/BANDIT-078/red-evidence.md`. |
| Stage 3 | `docs/work/BANDIT-078/implementation-evidence.md`, `docs/work/BANDIT-078/writer-report.md`, `docs/work/BANDIT-078/stage3-pm-review.md`, and role-run evidence. |
| Stage 4 | `docs/work/BANDIT-078/coderabbit-review.md`, `docs/work/BANDIT-078/local-qwen-review.md`, risk classification, supply-chain gate, review-subject hash, browser smoke evidence, finding dispositions when needed, and `docs/work/BANDIT-078/review-evidence.md`. |
| Stage 5 | `docs/work/BANDIT-078/uat-approval.md`, `docs/work/BANDIT-078/landing-verdict.md`, `docs/work/BANDIT-078/landing-action.md`, land-check output, and current source/evidence commit. |
| Stage 6 | `docs/work/BANDIT-078/retrospective.md`, improvement or no-action disposition artifacts, `.bandit/bootstrap-gaps.json` if changed, `docs/roadmap/CURRENT_CONTEXT.md`, `docs/roadmap/ROADMAP.md`, `STATUS.md`, and final validation output. |
...
- Permanent Test Ownership Boundary: Stage 3 Writer has zero authority to
  ...
  helpers, fixtures, RED evidence, or acceptance mappings for `BANDIT-078`.
...
- Operator owns product UAT approval and any product, policy, business, cost,
...
- `npm run bandit -- coderabbit-review pre-pr BANDIT-078 --base origin/main`
- `npm run bandit -- qwen-review BANDIT-078`
- `npm run bandit -- review-subject-hash BANDIT-078`
- `npm run bandit -- land-check BANDIT-078`
- `git diff --check`
...
mobile text fit, disabled states, focus order, source-link reachability, and no
overlap or truncation in dense action groups.
...
| Product UAT before landing | non_blocking | `docs/work/BANDIT-078/brief.md` requires CLI-owned product UAT before landing; not required before Stage 2. |
...
- Missing, stale, contradictory, or blocking formation evidence rejected by
  `work-item-pm start`.
- RED evidence lacks acceptance mapping, test ownership, or Stage 3 test-edit
  refusal.
...
- Browser code invokes CLI commands, writes repo artifacts, records UAT,
  ...
  guarded execution, or makes browser state canonical.
...
- Do not implement browser-side CLI execution, local API endpoints, live
  polling, websocket updates, server-side workflow actions, guarded action
  execution, State Index persistence, SQLite, browser storage as workflow
  state, cross-repo aggregation, scheduler execution, claim leases,
  work-surface reservations, worktree lifecycle, PR/CI orchestration,
  automatic merge, push, deploy, or production canary behavior.
- Do not choose local API shape, State Index timing, hosted packaging, external
  ...
  package-manager script changes, or live action queue semantics.
- Do not let generated JSON, preview files, fixture data, browser state, local
  cache, screenshots, static HTML, CSS, UI component state, or the
  action-eligibility view model become canonical workflow authority.
- Do not make the UI approve product UAT, policy overrides, business
  ...
  or landing safety.
- Do not replace current cockpit status, session-context, coordination, review,
  landing, UAT, retrospective, bootstrap-gap, risk-classification,
  supply-chain, artifact-create, work-item-create, or Trust Verifier authority.
- Do not implement the later Improvement Health Surface, local queue
  management, historical analytics, State Index, live action execution, or
  broader Phase 8 cockpit product scope inside this slice.

## 0. Context And Boundary

| Check | Verdict | Evidence |
| --- | --- | --- |
| Required reads complete | pass | Read `AGENTS.md`, `CONTEXT.md`, `docs/roadmap/CURRENT_CONTEXT.md`, `docs/roadmap/ROADMAP.md`, `docs/plans/BOOTSTRAP_METHODOLOGY.md`, `CLEAN_CODE.md`, `docs/verification/STAGE_RUBRICS.md`, `STATUS.md`, `docs/work/BANDIT-078/brief.md`, `docs/work/BANDIT-078/coordination-log.jsonl`, and `docs/templates/work-item-pm-plan.md`. |
...
| Routing surfaces agree | pass | `CURRENT_CONTEXT.md`, `ROADMAP.md`, `STATUS.md`, `cockpit status --json`, and `session-context current --json` identify `BANDIT-078` and plan mode as the next action. |
| Prior Work Item closed | pass | `docs/work/BANDIT-077/coordination-log.jsonl` records `closed`; `docs/work/BANDIT-077/landing-action.md` records local landing evidence; `docs/work/BANDIT-077/retrospective.md` exists. |
...
| Check | Verdict | Evidence |
| --- | --- | --- |
| Brief exists and satisfies Stage 1 | pass | `docs/work/BANDIT-078/brief.md` includes product work, source provenance, scope, out-of-scope, acceptance criteria, test plan, CLEAN_CODE.md evidence, role boundaries, stage capability scope, expected files, and operator-input status. |
| Qwen formation review exists and is not blocking | pass | `docs/work/BANDIT-078/qwen-formation-review.md` records `verdict: pass` and `findings_status: none` through the authorized MLX adapter route. |
| CodeRabbit formation review exists and is not blocking | pass | `docs/work/BANDIT-078/coderabbit-formation-review.md` records `verdict: pass`, terminal `review_completed`, and zero findings. |
| Aggregate formation review exists and is not blocking | pass | `docs/work/BANDIT-078/formation-review.md` records `verdict: pass` and no open findings. |
| Coordination log records `formation_approved` | pass | `docs/work/BANDIT-078/coordination-log.jsonl` sequence 2 records `state":"formation_approved"`. |
...
| RED evidence maps to acceptance criteria | pass | This plan requires RED tests for action derivation, availability, fail-closed disabled rendering, browser authority boundaries, source traceability, responsive behavior, and accessibility states before Stage 3. |
...
| Focused tests pass or bootstrap gap recorded | pass | Verification Commands list focused cockpit tests, `npm test` when shared surfaces are touched, and `npm run typecheck`. |
| Implementation evidence and Writer report required | pass | Required Evidence lists `implementation-evidence.md`, `writer-report.md`, `stage3-pm-review.md`, and role-run evidence. |
...
| Claude 15-minute window preserved | pass | Stop Conditions require giving Claude 15 minutes unless it exits or a hard blocker appears. |

## 4. Stage 4 Review Checklist

| Check | Verdict | Evidence |
| --- | --- | --- |
| CodeRabbit review or provider-refusal evidence | pass | Verification Commands include `npm run bandit -- coderabbit-review pre-pr BANDIT-078 --base origin/main`; brief allows honest timeout/refusal evidence. |
| Local Qwen review | pass | Verification Commands include `npm run bandit -- qwen-review BANDIT-078`; Local Qwen route is limited to `.bandit/reviewers/local-qwen.json` through `bin/omlx-chat-completions.mjs`. |
...
| Supply-chain gate when applicable | pass | Required Evidence includes supply-chain gate because browser-facing and action-authority presentation surfaces may be touched. |
| Findings repaired or dispositioned | pass | Stage 4 stop conditions block unresolved findings or stale evidence. |
| Aggregate review current for subject | pass | Verification Commands include `npm run bandit -- review-subject-hash BANDIT-078` and aggregate `review-evidence.md`. |
...
| Check | Verdict | Evidence |
| --- | --- | --- |
| Landing verdict exists before land-check closeout | pass | Required Evidence lists `docs/work/BANDIT-078/landing-verdict.md`. |
| Feature UAT handled | pass | Brief and this plan require CLI-owned product UAT evidence before landing. |
| `land-check` passes | pass | Verification Commands include `npm run bandit -- land-check BANDIT-078`. |
| Landing action records commit SHA | pass | Required Evidence lists `docs/work/BANDIT-078/landing-action.md`; landing path is local-record after clean source/evidence commit and source-head/hash refresh. |
...
| Check | Verdict | Evidence |
| --- | --- | --- |
| Retrospective exists | pass | Required Evidence lists `docs/work/BANDIT-078/retrospective.md`. |
