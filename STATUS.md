...
Stage 4 pre-PR CodeRabbit evidence for `BANDIT-056` is recorded in `docs/work/BANDIT-056/coderabbit-review.md` and `docs/specs/BANDIT-056-coderabbit-review-output.json` at source head `fabcd55d7bd31e388708749b481a2a0634de9855`. The focused CodeRabbit refresh returned two open findings.

The next required action is to repair or explicitly disposition the two open focused CodeRabbit findings before Local Qwen, aggregate Stage 4 review, landing, closeout, creating another work item, or unrelated Phase 8 work.
...
- `BANDIT-056` - Evidence Freshness SLOs (active - CodeRabbit findings next)
