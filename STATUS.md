...
`BANDIT-053` is active as the bootstrap-gap chore for `BANDIT-GAP-AGENT-OBSERVABILITY-TRACES`. Its structured creation spec, Stage 1 brief, Stage 2 RED evidence, Stage 3 implementation command surface, passing pre-PR CodeRabbit Stage 4 evidence, Local Qwen non-blocking evidence with PM disposition, and aggregate Stage 4 review evidence are recorded, and `.bandit/bootstrap-gaps.json` links the gap as `active_chore`.

The next required action is to write the Stage 5 landing verdict for `BANDIT-053`, including risk-classification and supply-chain gate evidence.
